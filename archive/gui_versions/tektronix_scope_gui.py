...
class ScopeGui(tk.Tk):
    ...
    def _build_layout(self) -> None:
        ...
        ttk.Entry(res_row, textvariable=self.resource_var).pack(side=tk.LEFT, fill=tk.X, expand=True, padx=(0, 8))
        ...
        ttk.Button(res_row, text="Test IDN", style="Accent.TButton", command=self.test_connection).pack(side=tk.LEFT)
        ttk.Button(res_row, text="List VISA", command=self.list_visa_resources).pack(side=tk.LEFT, padx=(8, 0))
    ...
    def _build_image_preview(self, parent: tk.Widget) -> None:
        ...
        self.preview_label.grid(row=1, column=0, sticky="nsew", padx=14, pady=(8, 14))
    ...
    def list_visa_resources(self) -> None:
        def job():
            ...
            self.after(0, lambda: messagebox.showinfo(APP_TITLE, text))
            return None

        self._run_job("Listing VISA resources", job)
