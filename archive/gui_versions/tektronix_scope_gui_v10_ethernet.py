...
Version: v10 Ethernet + USB/VISA support
...
- Right-side tabbed controls for labels, trigger, settings, and log.
...
class ScopeGui(tk.Tk):
    ...
    def _build_layout(self) -> None:
        ...
        visa_row = ttk.Frame(resource_card, style="Card.TFrame")
        visa_row.pack(fill=tk.X, padx=14, pady=(0, 6))
        ttk.Label(visa_row, text="VISA resource", style="Card.TLabel").pack(side=tk.LEFT, padx=(0, 8))
        ...
        ttk.Button(visa_row, text="Refresh VISA list", command=self.list_visa_resources).pack(side=tk.LEFT)

        eth_row = ttk.Frame(resource_card, style="Card.TFrame")
        eth_row.pack(fill=tk.X, padx=14, pady=(0, 6))
        ttk.Label(eth_row, text="Ethernet IP/host", style="Card.TLabel").pack(side=tk.LEFT, padx=(0, 8))
        ttk.Entry(eth_row, width=18, textvariable=self.eth_host_var).pack(side=tk.LEFT, padx=(0, 8))
        ttk.Label(eth_row, text="Protocol", style="Card.TLabel").pack(side=tk.LEFT, padx=(0, 8))
        ttk.Combobox(
            eth_row,
        ...
        ttk.Label(eth_row, text="Port", style="Card.TLabel").pack(side=tk.LEFT, padx=(0, 8))
        ttk.Entry(eth_row, width=7, textvariable=self.eth_port_var).pack(side=tk.LEFT, padx=(0, 8))
        ttk.Button(eth_row, text="Use Ethernet resource", command=self.apply_ethernet_resource).pack(side=tk.LEFT)

        generated_row = ttk.Frame(resource_card, style="Card.TFrame")
        generated_row.pack(fill=tk.X, padx=14, pady=(0, 6))
        ttk.Label(generated_row, text="Generated", style="Card.TLabel").pack(side=tk.LEFT, padx=(0, 8))
        ttk.Entry(generated_row, textvariable=self.generated_resource_var, state="readonly").pack(
            side=tk.LEFT, fill=tk.X, expand=True
        )

        action_row = ttk.Frame(resource_card, style="Card.TFrame")
        action_row.pack(fill=tk.X, padx=14, pady=(0, 14))
        ttk.Label(action_row, text="Timeout ms", style="Card.TLabel").pack(side=tk.LEFT, padx=(0, 8))
        ttk.Entry(action_row, width=9, textvariable=self.timeout_var).pack(side=tk.LEFT, padx=(0, 8))
        ttk.Button(action_row, text="Test IDN", style="Accent.TButton", command=self.test_connection).pack(side=tk.LEFT)
        ttk.Label(
            action_row,
            text="Ethernet VXI-11 uses TCPIP0::<ip>::INSTR. Socket uses TCPIP0::<ip>::4000::SOCKET.",
            style="Muted.TLabel",
    ...
    def _build_control_tabs(self, parent: tk.Widget) -> None:
        """Build right-side tabs for all controls and the log."""
