...
class LoggerOutputSession:
    """Own one run's CSV and/or DPO4LOG output writers."""
    ...
    def _refresh_bytes_written(self) -> None:
        self.bytes_written = sum(
            path.stat().st_size for path in self.paths if path.exists()
        )
    ...
    def close(self) -> None:
        ...
        for writer in (self.csv_writer, self.binary_writer):
            ...
            try:
                writer.close()
            except BaseException as exc:  # noqa: BLE001 - aggregate all close failures.
                errors.append(exc)
        self._closed = True
        self._refresh_bytes_written()
        if errors:
