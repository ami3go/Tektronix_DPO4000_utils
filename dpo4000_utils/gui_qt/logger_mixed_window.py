...
class QtScopeWindow(LoggerL6QtScopeWindow):
    """L6 Logger extended with synchronized mixed records."""
    ...
    def _build_logger_sources_card(self):
        ...
        if self.logger_mode_combo.findText(LoggerMode.MIXED.value) < 0:
            self.logger_mode_combo.addItem(LoggerMode.MIXED.value)
        return card

    def _logger_mode_changed(self, *_args) -> None:
        super()._logger_mode_changed(*_args)
        mode = self._logger_mode()
        mixed = mode is LoggerMode.MIXED
        ...
        if measurement_row is not None:
            measurement_row.setVisible(mixed or mode is LoggerMode.MEASUREMENTS)
        if bus_row is not None:
            bus_row.setVisible(mixed or mode is LoggerMode.BUS)
    ...
    def _logger_tick(self) -> None:
        ...
        try:
            ...
            if not isinstance(result, LoggerRecord):
                raise RuntimeError(
                    str(getattr(self, "_last_action", "Logger mixed capture failed"))
                )
            ...
            if result.metadata.get("partial"):
                self._append_log(
                    f"Logger mixed record {sequence} written as PARTIAL: {result.metadata}"
                )
            self.statusBar().showMessage(f"Mixed Logger record {sequence} written")
        except Exception as exc:  # noqa: BLE001 - fail closed on capture/output failure.
            if cancel.is_set() or self._logger_state is not LoggerState.RUNNING:
                self._logger_statistics.skipped += 1
                return
            ...
            try:
                ...
                self._append_log(
                    f"Logger output close after failure also failed: {close_exc}"
                )
            self._logger_output_session = None
            self._append_log(f"Logger L7 failed: {exc}")
        finally:
