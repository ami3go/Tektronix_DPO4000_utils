...
class WaveformPreamble:
    ...
    DPO4000, ``WFMOutpre:NR_Pt?`` is the number of points that ``CURVE?`` will
    actually transmit. It may be lower than the number of source-record points
    selected by ``DATA:START``/``DATA:STOP`` when the oscilloscope reduces the
    outgoing representation of a long record.
    """
...
def _read_preamble(scope: Any) -> WaveformPreamble:
    return WaveformPreamble(
        ...
        record_point_count=_query_int(
            scope,
            "WFMOUTPRE:NR_PT?",
            field="outgoing waveform point count",
        ),
...
def _validate_preamble(
    ...
    *,
    ...
    if preamble.record_point_count <= 0:
        raise DPOWaveformError(
            f"Scope reported invalid outgoing waveform count {preamble.record_point_count}."
        )
    if preamble.record_point_count > selected_record_points:
        raise DPOWaveformError(
            "Outgoing waveform count cannot exceed applied DATA range: "
            f"DATA range has {selected_record_points} source points, "
            f"WFMOUTPRE:NR_PT reports {preamble.record_point_count}."
        )
    if preamble.point_format != "Y":
        raise DPOWaveformError(
            "Envelope/min-max waveform point format is not silently flattened by the v0.6 API. "
            f"Scope reported PT_FMT={preamble.point_format!r}; use SAMPLE/HIRES/AVERAGE or "
            "add an explicit envelope-processing policy."
        )
    ...
    if not preamble.encoding.startswith("BIN"):
        raise DPOWaveformError(
            f"Scope preamble encoding {preamble.encoding!r} is not binary after requesting "
            f"{request_encoding}."
        )
    ...
    if preamble.binary_format != expected_format:
        raise DPOWaveformError(
            f"Scope binary format does not match request: expected {expected_format}, "
            f"got {preamble.binary_format!r}."
        )
    if sample_width > 1 and preamble.byte_order != expected_order:
        raise DPOWaveformError(
            f"Scope byte order does not match request: expected {expected_order}, "
            f"got {preamble.byte_order!r}."
        )
    ...
    if callable(query_binary_values):
        ...
        if not isinstance(values, array):
            try:
                ...
                raise DPOWaveformError(
                    "Binary CURVE decoder returned unsupported sample container "
                    f"{type(values)!r}."
                ) from exc
        return values
...
def _read_source_label(scope: Any, source: str) -> str:
    if source.startswith(("CH", "REF")):
        label = optional_query(
            scope,
            f"{source}:LABEL?",
            normalizer=normalize_scope_response_text,
        )
...
def read_waveform(scope: Any, request: WaveformRequest) -> WaveformData:
    ...
    _write_required(
        scope,
        f"DATA:STOP {requested_stop if requested_stop is not None else FULL_WAVEFORM_STOP}",
    )
    ...
    if start != requested_start:
        raise DPOWaveformError(
            f"Scope adjusted DATA:START from requested {requested_start} to {start}; "
            "request is outside the available waveform."
        )
    if requested_stop is not None and stop != requested_stop:
        raise DPOWaveformError(
            f"Scope adjusted DATA:STOP from requested {requested_stop} to {stop}; "
            "request exceeds the available waveform."
        )

    selected_record_points = stop - start + 1
    preamble = _read_preamble(scope)
    _validate_preamble(
        preamble,
        request_encoding=encoding,
        sample_width=sample_width,
        selected_record_points=selected_record_points,
    )
    transfer_count = preamble.record_point_count
    label = _read_source_label(scope, source)

    if encoding == "ASCII":
        if transfer_count > ASCII_MAX_POINTS:
            raise DPOWaveformError(
                "ASCII CURVE transfers above 1,000,000 outgoing points are not supported "
    ...
    if len(samples) != transfer_count:
        raise DPOWaveformError(
            "Waveform point-count mismatch: "
            f"WFMOUTPRE:NR_PT reports {transfer_count}, received {len(samples)} "
            f"for selected DATA range {start}..{stop}."
        )
...
def scale_waveform_samples(
    ...
    *,
    ...
    times = [
        x_origin + (index - point_offset) * x_increment
        for index in range(len(raw_samples))
    ]
    volts = [
        (sample - y_offset) * y_multiplier + y_zero
...
def validate_waveform_alignment(waveforms: Sequence[WaveformData]) -> None:
    ...
    for waveform in waveforms[1:]:
        if waveform.sample_count != reference.sample_count:
            raise DPOWaveformError(
                "Waveform sample-count mismatch: "
                f"{reference.source} has {reference.sample_count}, "
                f"{waveform.source} has {waveform.sample_count}."
            )
        if waveform.start_index != reference.start_index or waveform.stop_index != reference.stop_index:
            raise DPOWaveformError(
                f"Waveform transfer-range mismatch between {reference.source} "
                f"and {waveform.source}."
            )
...
def read_enabled_channel_waveforms(
    ...
    return (
        list(values[0].iter_times()),
        {w.display_name: list(w.iter_voltages()) for w in values},
    )
...
def write_waveforms_csv(
    ...
    with output.open("w", newline="", encoding="utf-8") as handle:
        ...
        writer.writerow(
            [
                f"Time ({values[0].preamble.x_unit or 's'})",
                *[w.display_name for w in values],
            ]
        )
        for i in range(values[0].sample_count):
            writer.writerow(
                [values[0].time_at(i), *[w.voltage_at(i) for w in values]]
            )
    return output
...
def save_enabled_channels_to_separate_csv(
    scope: Any,
    ...
    for waveform in read_enabled_waveforms(scope).values():
        written.append(
            write_waveform_csv(
                base.with_name(f"{base.name}_{waveform.source}.csv"),
                waveform,
            )
        )
    return written
