...
class HilSkip(RuntimeError):
    """Mark one HIL case as intentionally not applicable."""
...
class AutomationLoggerHilRunner:
    """Run isolated Automation and Logger qualification cases on a Probe Comp signal."""

    def __init__(self, config: HilConfig) -> None:
        self.config = config
        stamp = datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ")
        self.root = (config.output_dir / f"automation_logger_hil_{stamp}").resolve()
        self.root.mkdir(parents=True, exist_ok=False)
        self.artifact_root = self.root / "artifacts"
        self.error_root = self.root / "tracebacks"
        self.artifact_root.mkdir()
        self.error_root.mkdir()
    ...
    def _case(
        ...
        try:
            ...
            result = HilCaseResult(
                ...
                traceback_path=str(tb_path.relative_to(self.root)),
            )
        ...
        self._log(
            ...
            case_id,
            result.duration_s,
            result.detail or result.error_message,
        )
    ...
    def _fixture(self) -> str:
        ...
        low = self.config.expected_frequency_hz * (
            1 - self.config.frequency_tolerance_fraction
        )
        high = self.config.expected_frequency_hz * (
            1 + self.config.frequency_tolerance_fraction
        )
        ...
        while time.monotonic() < deadline:
            try:
                ...
                if (
                    low <= freq <= high
                    and self.config.min_vpp_v <= vpp <= self.config.max_vpp_v
                ):
                    return f"Probe Comp detected CH{ch}: {freq:.6g} Hz, {vpp:.6g} Vpp"
                last_detail = (
                    f"frequency={freq:g} Hz expected {low:g}..{high:g}; "
                    f"Vpp={vpp:g} V expected "
                    f"{self.config.min_vpp_v:g}..{self.config.max_vpp_v:g}"
                )
            except (TypeError, ValueError) as exc:
                last_detail = str(exc)
            time.sleep(0.2)
        raise AssertionError(
            f"Probe Comp preflight did not settle within 5 s: {last_detail}"
        )
    ...
    def _restore(self) -> str:
        ...
        if after.get("setup") == self.baseline.get("setup"):
            return "Exact *LRN? match restored"
        return "Baseline reapplied; *LRN? differs. Compare before/after JSON"

    def _a1(self) -> tuple[str, list[dict[str, Any]]]:
        if not self.config.hardcopy:
            raise HilSkip("Hardcopy disabled")
        ...
        path = self.artifact_root / "automation" / "a1_periodic.png"
        ...
        controller.stop()
        return "A1 capture + no-overlap passed", [self._png(path)]

    def _a2(self) -> tuple[str, list[dict[str, Any]]]:
        if not self.config.hardcopy:
            raise HilSkip("Hardcopy disabled")
        ...
        if not wait.completed:
            raise AssertionError(f"Fresh Single failed: {asdict(wait)}")
        path = self.artifact_root / "automation" / "a2_trigger.png"
        self._scope().save_image_path(path)
        return (
            f"A2 trigger={wait.trigger_state} elapsed={wait.elapsed_s:.3f}s",
            [self._png(path)],
        )

    def _a3(self) -> tuple[str, list[dict[str, Any]]]:
        if not self.config.hardcopy:
            raise HilSkip("Hardcopy disabled")
        ...
        result = acquire_trigger_bundle(
            ...
            poll_interval_s=0.1,
            timeout_s=self.config.trigger_timeout_s,
            image_path=root / "a3_bundle.png",
            csv_path=root / "a3_bundle.csv",
        )
        if not result.artifacts_complete:
            raise AssertionError(f"A3 incomplete: {asdict(result)}")
        return (
            f"A3 points={result.point_count} trigger={result.trigger_state}",
            [self._png(result.image_path), self._csv(result.csv_path)],
        )

    def _a4(self) -> tuple[str, list[dict[str, Any]]]:
        result = save_full_record_csv(
            self._scope(),
            self.artifact_root / "automation" / "a4_waveform.csv",
        )
        if not result.success:
            raise AssertionError(result.error)
        return f"A4 points={result.point_count}", [self._csv(result.csv_path)]

    def _a5(self) -> tuple[str, list[dict[str, Any]]]:
        path = self.artifact_root / "automation" / "a5_measurements.csv"
        started = datetime.now(timezone.utc)
        last = None
        for _ in range(3):
            last = append_measurement_row(
                self._scope(),
                path,
                (1, 2),
                run_started_utc=started,
            )
            if not last.success or last.slot_errors:
                raise AssertionError(str(last))
            time.sleep(0.1)
        assert last is not None
        return f"A5 three rows; last={last.values}", [self._csv(path, 4)]

    def _a6(self) -> tuple[str, list[dict[str, Any]]]:
        freq = self._numeric(self._scope().read_measurement_value(1))
        evaluator = ConditionalEvaluator(
            ConditionalCaptureConfig(
                slot=1,
                operator=">",
                threshold=max(1.0, freq * 0.5),
                consecutive_matches=1,
                cooldown_s=1.0,
            )
        )
        result = run_conditional_poll(
            self._scope(),
            evaluator,
        ...
        if not result.captured or result.artifacts is None:
            raise AssertionError(str(result))
        return (
            f"A6 fired value={result.evaluation.value:g}",
            [self._csv(result.artifacts.csv_path)],
        )

    def _a7(self) -> tuple[str, list[dict[str, Any]]]:
        config = BurstConfig(
            count=2,
            delay_s=0.0,
            action=ArtifactAction.CSV,
        ...
        for index in range(1, 3):
            result = run_burst_event(
                ...
                csv_path=(
                    self.artifact_root
                    / "automation"
                    / f"a7_burst_{index:02d}.csv"
                ),
            )
            if not result.success or result.artifacts is None:
                raise AssertionError(str(result))
            artifacts.append(self._csv(result.artifacts.csv_path))
        return "A7 two-event fresh-Single CSV burst passed", artifacts

    def _a8(self) -> str:
        tracker = RunLimitTracker(RunLimits(max_events=2, max_duration_s=10.0))
        tracker.start(100.0)
        assert not tracker.status(1, 101.0).reached
        status = tracker.status(2, 102.0)
        assert status.reached
        return f"A8 exact boundary: {status.reason}"

    def _a9(self) -> tuple[str, list[dict[str, Any]]]:
        root = self.artifact_root / "automation" / "a9_retention"
        root.mkdir(parents=True, exist_ok=True)
        first = root / "old.txt"
        second = root / "new.txt"
        first.write_bytes(b"old" * 100)
        second.write_bytes(b"new" * 100)
        register_retention_event(
            root,
            "old",
            (first,),
        ...
        plan = plan_retention(root, RetentionPolicy(keep_last_events=1))
    ...
    def _a10(self) -> tuple[str, list[dict[str, Any]]]:
        path = self.artifact_root / "automation" / "a10_profile.json"
        profile = AutomationProfile(
            "Probe Comp HIL",
            {"mode": "Image on Trigger", "channel": self.config.channel},
        )
    ...
    def _a11(self) -> str:
        ...
        assert before == after
        return (
            "A11 disconnect/reconnect primitive passed; "
            "physical cable fault injection not automated"
        )

    def _a12(self) -> tuple[str, list[dict[str, Any]]]:
        root = self.artifact_root / "automation" / "a12_report"
        reporter = AutomationRunReporter(
            root=root,
            mode="HIL",
            config={"fixture": "Probe Comp -> CH1"},
            resource=self.config.resource,
            idn=self.idn,
        ...
        summary = reporter.finalize(
            ...
            counters={"successful": 1, "failed": 0},
        )
        assert json.loads(summary.read_text(encoding="utf-8"))["event_count"] == 1
        return (
            "A12 durable event + summary passed",
            [
                self._file(reporter.event_jsonl_path),
                self._csv(reporter.event_csv_path),
                self._file(summary),
            ],
        )

    def _logger_waveform(self) -> tuple[str, list[dict[str, Any]]]:
        root = self.artifact_root / "logger" / "waveform"
        config = LoggerConfig(
            mode=LoggerMode.WAVEFORM,
            interval_s=0.1,
            waveform_sources=(f"CH{self.config.channel}",),
            point_count=self.config.waveform_points,
        )
        output = LoggerOutputSession(
            root,
        ...
        try:
            for sequence in (1, 2):
                output.append(capture_logger_record(self._scope(), config, sequence))
        finally:
            output.close()
        assert output.records_written == 2
        assert output.rotation_count >= 1
        artifacts = []
        for path in output.paths:
            ...
            if path.suffix == ".dpo4log":
                ...
                if not scan.clean_end or scan.truncated or scan.record_count < 1:
                    raise AssertionError(str(scan))
        return (
            f"Waveform BOTH format records=2 rotations={output.rotation_count}",
            artifacts,
        )

    def _logger_measurements(self) -> tuple[str, list[dict[str, Any]]]:
        ...
        try:
            for sequence in (1, 2, 3):
                last = capture_logger_record(self._scope(), config, sequence)
                output.append(last)
        finally:
            output.close()
        assert last is not None
        assert not last.measurement_errors
        return (
            f"Measurement Logger records=3 last={dict(last.measurements)}",
            [self._file(path) for path in output.paths],
        )

    def _logger_mixed(self) -> tuple[str, list[dict[str, Any]]]:
        ...
        if bool(record.metadata.get("partial")):
            raise AssertionError(str(record.metadata))
        output = LoggerOutputSession(
            root,
            LoggerOutputFormat.BOTH,
            mode=LoggerMode.MIXED,
        ...
        try:
            ...
            output.close()
        assert len(record.waveforms) == 1
        assert len(record.measurements) == 2
        return (
            f"Mixed fresh-Single trigger={record.metadata.get('trigger_state')}",
            [self._file(path) for path in output.paths],
        )

    def _logger_math(self) -> str:
        self._scope().configure_math(
            MathConfig(display=True, define=f"CH{self.config.channel}")
        )
        config = LoggerConfig(
            mode=LoggerMode.WAVEFORM,
            interval_s=0.1,
            waveform_sources=("MATH",),
            point_count=self.config.waveform_points,
        )
        record = capture_logger_record(self._scope(), config, 1)
        assert len(record.waveforms) == 1
        assert record.waveforms[0].source == "MATH"
        return f"MATH Logger captured {record.waveforms[0].sample_count} points"

    def _logger_bus(self) -> str:
        if not self._scope().supports_decoded_bus_events():
            raise HilSkip(
                "Decoded BUS extraction is unqualified; "
                "Probe Comp -> CH1 cannot qualify serial decode"
            )
        raise HilSkip("BUS decode requires real serial-bus stimulus, not Probe Comp")

    def _logger_buffer_health(self) -> tuple[str, list[dict[str, Any]]]:
        policy = BufferPolicy(
            max_records=2,
            max_bytes=10_000_000,
            stop_after_overflows=2,
        )
        buffer = BoundedRecordBuffer(policy)
        now = datetime.now(timezone.utc).isoformat()
        r1, r2, r3 = (
            LoggerRecord(sequence=index, captured_utc=now)
            for index in (1, 2, 3)
        )
        assert buffer.try_put(r1)
        assert buffer.try_put(r2)
        assert not buffer.try_put(r3)
        assert buffer.pop_left() is r1
        capture = LoggerHealthAccumulator()
        capture.note_capture(r2, 0.01)
        snapshot = BufferSnapshot(
            queued_records=buffer.queued_records,
            queued_bytes=buffer.queued_bytes,
            written_records=1,
            bytes_written=100,
            total_write_s=0.01,
        )
        health = compute_logger_health(
            capture.snapshot(),
            snapshot,
            elapsed_s=1.0,
        ...
        worker = LoggerWriterWorker(
            lambda: LoggerOutputSession(
                root,
                LoggerOutputFormat.BINARY,
                mode=LoggerMode.MEASUREMENTS,
                measurement_slots=(1, 2),
                run_metadata={"source": "HIL writer thread"},
            ),
            BufferPolicy(
                max_records=4,
                max_bytes=10_000_000,
        ...
        for sequence in (10, 11):
            assert worker.try_enqueue(
                capture_logger_record(self._scope(), config, sequence)
            )
        worker.request_stop(drain=True)
        assert worker.wait(10.0)
        writer_snapshot = worker.snapshot()
        if writer_snapshot.error or writer_snapshot.written_records != 2:
            raise AssertionError(f"Logger writer thread failed: {writer_snapshot}")
        ...
        for path in paths:
            ...
            if not scan.clean_end or scan.truncated or scan.record_count < 1:
                raise AssertionError(
                    f"Invalid writer-thread DPO4LOG {path}: {scan}"
                )
            artifacts.append(self._file(path))
        return (
            "Buffer overflow + health + writer thread passed; "
            f"written={writer_snapshot.written_records}; "
            f"effective_rate={health.effective_records_per_s:.3f}/s",
            artifacts,
        )

    def _logger_recovery(self) -> str:
        before = self._scope().query_identity().strip()
        self._scope().disconnect()
        self._scope().connect()
        after = self._scope().query_identity().strip()
        assert before == after
        return (
            "Logger recovery reconnect primitive passed; "
            "physical transport-fault injection is intentionally not automatic"
        )

    def _logger_profile(self) -> tuple[str, list[dict[str, Any]]]:
        root = self.artifact_root / "logger" / "profile"
        root.mkdir(parents=True, exist_ok=True)
        path = root / "probe_comp.json"
        profile = LoggerProfile(
            name="Probe Comp",
            config={
                "mode": LoggerMode.WAVEFORM.value,
                "interval_s": 1.0,
                "waveform_sources": [f"CH{self.config.channel}"],
                "measurement_slots": [],
                "bus_slots": [],
                ...
                "sample_width": 2,
                "point_count": self.config.waveform_points,
                "output_format": LoggerOutputFormat.BOTH.value,
        ...
        save_logger_profile(path, profile)
        loaded = load_logger_profile(path)
        assert loaded.name == profile.name
        return "Logger profile round-trip passed", [self._file(path)]

    def _logger_retention(self) -> tuple[str, list[dict[str, Any]]]:
        root = self.artifact_root / "logger" / "retention"
        root.mkdir(parents=True, exist_ok=True)
        first = root / "segment1.dpo4log"
        second = root / "segment2.dpo4log"
        first.write_bytes(b"one" * 100)
        second.write_bytes(b"two" * 100)
        ...
        assert second.exists()
        return (
            f"Logger retention reclaimed={result.reclaimed_bytes}B",
            [self._file(second)],
        )

    def _logger_report(self) -> tuple[str, list[dict[str, Any]]]:
        ...
        checkpoint = reporter.checkpoint(
            {"records_captured": 1, "records_written": 1},
        ...
        return (
            "Logger event/checkpoint/final report passed",
            [
                self._file(reporter.event_jsonl_path),
                self._csv(reporter.event_csv_path),
    ...
    def _automation_cases(self) -> None:
        for case_id, title, callback in [
            ...
            self._case(case_id, title, "automation", callback)

    def _logger_cases(self) -> None:
        for case_id, title, callback in [
            (
                "logger-waveform",
                "Waveform CSV + DPO4LOG + rotation",
                self._logger_waveform,
            ),
            (
                "logger-measurements",
                "Measurement CSV + DPO4LOG",
                self._logger_measurements,
            ),
            ...
            (
                "logger-buffer-health",
                "Bounded buffer + health + writer thread",
                self._logger_buffer_health,
            ),
            (
                "logger-recovery",
                "Logger reconnect/recovery primitive",
                self._logger_recovery,
            ),
            ("logger-profile", "Logger profile", self._logger_profile),
            ("logger-retention", "Logger retention", self._logger_retention),
            (
                "logger-report",
                "Logger report/checkpoint",
                self._logger_report,
            ),
        ]:
            self._case(case_id, title, "logger", callback)
    ...
    def _write_report(self) -> None:
        ...
        lines = [
            "# DPO4000 Automation + Logger Probe Comp HIL",
            "",
            f"**Status:** {payload['status']}",
            f"**Resource:** `{self.config.resource}`",
            f"**IDN:** `{self.idn}`",
            (
                f"**PASS/FAIL/SKIP:** {counts[PASS]} / "
        ...
        for result in self.results:
            detail = (
                result.detail or f"{result.error_type}: {result.error_message}"
            ).replace("|", "\\|").replace("\n", " ")
            lines.append(
                f"| `{result.case_id}` | {result.group} | **{result.status}** | "
        ...
        lines += [
            ...
            "## Debug bundle",
            "",
            "- `hil_run.log`: chronological trace",
            ...
            (
                "- `scope_setup_before.json` / `scope_setup_after_restore.json`: "
    ...
    def _zip(self) -> Path:
        return Path(
            shutil.make_archive(
                str(self.root.parent / f"{self.root.name}_diagnostic_bundle"),
                "zip",
                root_dir=self.root,
                base_dir=".",
            )
    ...
    def run(self) -> tuple[int, Path]:
        ...
        self._log(
            "Resource=%s suite=%s waveform_points=%d",
            self.config.resource,
            self.config.suite,
            self.config.waveform_points,
        )
        try:
            ...
            self._case(
                "fixture-probe-comp",
                "Probe Comp signal preflight",
                "fixture",
                self._fixture,
            )
            if self.results[-1].status == PASS:
                if self.config.suite in {"all", "automation"}:
                    self._automation_cases()
                if self.config.suite in {"all", "logger"}:
                    self._logger_cases()
            else:
                self._log(
                    "Fixture preflight failed; hardware-dependent cases not run"
                )
        except BaseException as exc:
            path = self.error_root / "runner_fatal.txt"
            ...
            if self.scope is not None:
                try:
                    ...
                    self._logger.exception("Cleanup disconnect failed")
            self._write_report()
            zip_path = self._zip()
        ...
        exit_code = 1 if any(result.status == FAIL for result in self.results) else 0
        return exit_code, zip_path
